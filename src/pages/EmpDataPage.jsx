...
const EmpDataPage = () => {
    ...
    const [activeMenu, setActiveMenu] = useState('empData');
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const { data, error } = await supabase.from('emp_data').select('*').order('id', { ascending: false }).limit(5000);
            ...
            setTableData(rows);
        } catch (error) { console.error('Error fetching emp_data:', error); }
    ...
    const handleExcelDownload = () => {
        ...
        xml += '<Worksheet ss:Name="Employee Data"><Table>';
        ...
        const a = document.createElement('a'); a.href = url; a.download = `Employee_Data_${new Date().toISOString().slice(0, 10)}.xls`;
                ...
                <div className="content" style={{ padding: '10px 14px' }}>
                    <div className="ss-container">
                            ...
                            <div className="ss-toolbar-left">
                                <div className="dash-title-row" style={{ marginRight: '15px' }}>
                                    ...
                                    <div className="dash-title-text" style={{ marginLeft: '10px' }}>
                                        <h2 className="dash-title" style={{ fontSize: '15px', marginBottom: '2px' }}>Employee Data</h2>
                                        <div className="dash-welcome" style={{ fontSize: '10.5px' }}><span>Employee records from Supabase</span></div>
                                    </div>
                                </div>
                        ...
                        <div className="ss-table-card">
                            ...
                            {isLoading ? (<div className="ss-loading"><div className="ss-spinner"></div><p>Loading Employee Data...</p></div>)
                            : filteredData.length === 0 ? (<div className="ss-empty"><div className="ss-empty-icon">👥</div><p>No employee records found</p></div>)
                            : (<>
