...
const ShieldPage = () => {
    ...
    const [activeMenu, setActiveMenu] = useState('shield');
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const { data, error } = await supabase.from('shield').select('*').order('id', { ascending: false }).limit(5000);
            ...
            setTableData(rows);
        } catch (error) { console.error('Error fetching shield:', error); }
    ...
    const handleExcelDownload = () => {
        ...
        xml += '<Worksheet ss:Name="Shield"><Table>';
        ...
        const a = document.createElement('a'); a.href = url; a.download = `Shield_${new Date().toISOString().slice(0, 10)}.xls`;
                ...
                <div className="content" style={{ padding: '10px 14px' }}>
                    <div className="ss-container">
                        <div className="ss-toolbar">
                            <div className="ss-toolbar-left">
                                <div className="dash-title-row" style={{ marginRight: '15px' }}>
                                    ...
                                    <span className="dash-icon" style={{ background: 'linear-gradient(135deg, #14b8a6, #0d9488)', width: '30px', height: '30px', padding: '6px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                        <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>
                                    </span>
                                    <div className="dash-title-text" style={{ marginLeft: '10px' }}>
                                        <h2 className="dash-title" style={{ fontSize: '15px', marginBottom: '2px' }}>Shield</h2>
                                        <div className="dash-welcome" style={{ fontSize: '10.5px' }}><span>Shield data from Supabase</span></div>
                                    </div>
                                </div>
                        ...
                        <div className="ss-table-card">
                            ...
                            {isLoading ? (<div className="ss-loading"><div className="ss-spinner"></div><p>Loading Shield...</p></div>)
                            : filteredData.length === 0 ? (<div className="ss-empty"><div className="ss-empty-icon">🛡️</div><p>No shield records found</p></div>)
                            : (<>
