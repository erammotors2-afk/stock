...
const FtdRetailPage = () => {
    ...
    const [activeMenu, setActiveMenu] = useState('ftdRetail');
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const { data, error } = await supabase.from('ftd_retail').select('*').order('id', { ascending: false }).limit(5000);
            ...
            setTableData(rows);
        } catch (error) { console.error('Error fetching ftd_retail:', error); }
    ...
    const handleExcelDownload = () => {
        ...
        xml += '<Worksheet ss:Name="FTD Retail"><Table>';
        ...
        const a = document.createElement('a'); a.href = url; a.download = `FTD_Retail_${new Date().toISOString().slice(0, 10)}.xls`;
                ...
                <div className="content" style={{ padding: '10px 14px' }}>
                    <div className="ss-container">
                        <div className="ss-toolbar">
                            <div className="ss-toolbar-left">
                                <div className="dash-title-row" style={{ marginRight: '15px' }}>
                                    ...
                                    <span className="dash-icon" style={{ background: 'linear-gradient(135deg, #84cc16, #65a30d)', width: '30px', height: '30px', padding: '6px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                        <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12" /></svg>
                                    </span>
                                    <div className="dash-title-text" style={{ marginLeft: '10px' }}>
                                        <h2 className="dash-title" style={{ fontSize: '15px', marginBottom: '2px' }}>FTD Retail</h2>
                                        <div className="dash-welcome" style={{ fontSize: '10.5px' }}><span>FTD Retail data from Supabase</span></div>
                                    </div>
                                </div>
                        ...
                        <div className="ss-table-card">
                            ...
                            {isLoading ? (<div className="ss-loading"><div className="ss-spinner"></div><p>Loading FTD Retail...</p></div>)
                            : filteredData.length === 0 ? (<div className="ss-empty"><div className="ss-empty-icon">📊</div><p>No FTD retail records found</p></div>)
                            : (<>
